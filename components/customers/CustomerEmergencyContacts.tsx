...
import { User, Mail, Phone, Users, Trash2 } from 'lucide-react'
        ...
        <>
          ...
          <div className="md:hidden space-y-3">
            {pagedItems.map(item => (
              <MobileCard
                key={item.id}
                header={<><User size={18} className="text-[#6D6D6D]" /><span className="truncate">{item.emergency_contact.first_name} {item.emergency_contact.last_name}</span></>}
                action={<button onClick={() => { setSelectedEcToDelete(item); setDeletePopup(true) }} aria-label="Delete" className="text-red-600"><Trash2 size={18} /></button>}
                rows={[
                  { icon: <Mail size={18} />, value: item.emergency_contact.email ?? '' },
                  { icon: <Phone size={18} />, value: item.emergency_contact.phone ?? '' },
                  { icon: <Users size={18} />, value: item.relationship ?? '' },
                ]}
              />
            ))}
            {pagedItems.length === 0 && <div className="py-8 text-center text-gray-500">No Emergency Contacts Found</div>}
          </div>
        </>
      )}
