...
interface Row { sessionDefaultChannelGrouping: string; sessions: number; conversions: number; effect: number }
...
/** Nuxt GroupingTable — "Traffic Sources" (channel grouping), 5 rows/page. */
export function GroupingTable() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await getSecure<Row[]>(SECURE_ENDPOINTS.LEADS_CHANEL_GROUPING, {})
      ...
      <div className="flex-1 overflow-x-auto">
        <table className="w-full text-left text-xs">
          <thead className="bg-gray-50 text-gray-600"><tr>
            <th className="px-3 py-2 font-medium">Channel Grouping</th>
          ...
          <tbody>
            {pageItems.map((r, i) => (
              <tr key={i} className="border-t">
                <td className="px-3 py-2">{r.sessionDefaultChannelGrouping}</td>
            ...
            {pageItems.length === 0 && <tr><td colSpan={4} className="px-3 py-6 text-center text-gray-400">No data</td></tr>}
          </tbody>
        </table>
      </div>
